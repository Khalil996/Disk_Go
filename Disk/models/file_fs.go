...
type FileFs struct {
	...
	ChunkNum   int       `xorm:"bigint notnull default 0 'chunk_num' comment('分片数量')"`
	Url        string    `xorm:"varchar(255) notnull default '' 'url' comment('访问地址')"`
	Status     int       `xorm:"tinyint notnull default 0 'status' comment('文件状态，0：大文件未上传，1：大文件待合并，2：小文件未上传，3：上传成功')"` //
